...
#define CONFIG_ZYNQ_PS_CLK_FREQ	40000000UL
...
#define CONFIG_CPU_FREQ_HZ	660000000
...
#define CONFIG_SYS_TEXT_BASE	0x400000  /* Define in avnet-zedboard_uboot_defconfig !!!! */
#endif 
...
#define CONFIG_ENV_OFFSET	0x6C0000
#define CONFIG_ENV_SIZE	0x20000
#define CONFIG_ENV_SECT_SIZE	0x20000

/* PREBOOT */
#define CONFIG_PREBOOT	"echo U-BOOT for avnet-digilent-zedboard-2017_4;setenv preboot; echo; dhcp"
...
#define CONFIG_EXTRA_ENV_SETTINGS	\
	...
	"uboot_flash_addr=0x640000\0" \
	"uboot_size=0x80000\0"	\
	"bootenv=uEnv.txt\0" \
	"mmcboot=echo Copying Linux from MMC to RAM... && " \
		"fatload mmc 0:1 ${kernel_addr} ${kernel_img} && " \
		"fatload mmc 0:1 ${dtb_addr} ${dtb_img} && " \
		"echo Copying ramdisk... && " \
		"fatload mmc 0:1 ${ramdisk_addr} ${ramdisk_img} && " \
		"bootz ${kernel_addr} ${ramdisk_addr} ${dtb_addr}\0" \
	"burn_kernel=echo Download and burn Linux Kernel... &&" \
		"tftpboot ${kernel_addr} ${kernel_img} &&" \
		"fatwrite mmc 0:1 ${kernel_addr} ${kernel_img} ${filesize}\0" \
	"burn_ramdisk=echo Download and burn Linux Ramdisk... &&" \
		"tftpboot ${ramdisk_addr} ${ramdisk_img} &&" \
		"fatwrite mmc 0:1 ${ramdisk_addr} ${ramdisk_img} ${filesize}\0" \
	"burn_dtb=echo Download and burn device tree &&" \
		"tftpboot ${dtb_addr} ${dtb_img} &&" \
		"fatwrite mmc 0:1 ${dtb_addr} ${dtb_img} ${filesize}\0" \
	"burn_bit=echo Download and burn BIT &&" \
		"tftpboot ${bit_addr} ${bit_img} &&" \
		"sf probe 0 0 0 && " \
		"sf erase ${bit_flash_addr} ${bit_size} &&" \
		"sf write ${bit_addr} ${bit_flash_addr} ${bit_size}\0" \
	"burn_fsbl=echo Download and burn FSBL &&" \
		"tftpboot ${fsbl_addr} ${fsbl_img} &&" \
		"sf probe 0 0 0 && " \
		"sf erase ${fsbl_flash_addr} ${fsbl_size} &&" \
		"sf write ${fsbl_addr} ${fsbl_flash_addr} ${fsbl_size}\0" \
	"burn_uboot=echo Download and burn U-Boot &&" \
		"tftpboot ${uboot_addr} ${uboot_img} &&" \
		"sf probe 0 0 0 && " \
		"sf erase ${uboot_flash_addr} ${uboot_size} &&" \
