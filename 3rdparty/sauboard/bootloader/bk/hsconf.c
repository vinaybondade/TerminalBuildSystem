...
int set_hiSkyId(config_param_info_t *infoPtr, const char *id)
{
    ...
    memset(hiskyidStr, 0, sizeof(hiskyidStr));
    sprintf (hiskyidStr, "%s%s%s", infoPtr->paramTag->beginTag, id, infoPtr->paramTag->endTag); 
    
    memset(infoPtr->ramAddr, 0, infoPtr->size);
    strcpy(infoPtr->ramAddr, hiskyidStr);
...
int set_rfSettings(config_param_info_t *infoPtr, const char *settings)
{
    ...
    iter1 += strlen("system_serial_number") + 3; // advance for strlen("\" : \"") bytes
    iter2 = strstr(iter1, "\"");
