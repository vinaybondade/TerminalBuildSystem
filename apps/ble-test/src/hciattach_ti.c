				...
				unsigned long buf_size, uint16_t *action_type)
{
	...
	if (action_hdr.size > buf_size) {
		printf("bts_next_action: not enough space to read next action\n");
		return 0;
	}

	nread = fread(action_buf, sizeof(uint8_t), action_hdr.size, fp);
	if (nread != (action_hdr.size)) {
		printf("bts_next_action: fread failed to read next action\n");
		return 0;
	}
...
{
	printf("%s: texas: changing baud rate to %u, flow control to %u\n", __func__,
...
static int brf_send_command_socket(int fd, struct bts_action_send *send_action)
{
	...
	if (hci_send_req(fd, &rq, 15) < 0) {
		printf("%s:Cannot send hci command to socket\n", __func__);
		return -1;
	}

	/* verify success */
	if (response[0]) {
		printf("%s: Responce is incorrect, return -1\n", __func__);
		errno = EIO;
		return -1;
	}
								...
								long size)
{
	...
	if (size != write(fd, send_action, size)) {
		printf("%s:Texas: Failed to write action command\n", __func__);
		return -1;
	}
	...
	if (ret < 0) {
		printf("%s:texas: failed to read command response\n", __func__);
		return -1;
	}

	/* verify success */
	if (ret < 7 || 0 != response[6]) {
		printf("%s:TI init command failed.\n", __func__);
		errno = EIO;
		return -1;
	}
				...
				int fd, int *speed, struct termios *ti, int hcill_installed)
{
	...
	case ACTION_REMARKS:
		...
		break;
	default:
		printf("brf_init: unknown firmware action type (%d)\n", brf_type);
		break;
	}
...
static int brf_do_script(int fd, int *speed, struct termios *ti, const char *bts_file)
{
	...
	if (0 == hcill_installed) {
		printf("Sending script to serial device\n");
		brf_script_file = bts_load_script(bts_file, &vers );
		if (!brf_script_file) {
			printf("Warning: cannot find BTS file: %s\n",
					bts_file);
			return 0;
		}

		printf( "Loaded BTS script version %u\n", vers );

		brf_size = bts_fetch_action(brf_script_file, brf_action,
						sizeof(brf_action), &brf_type);
		if (brf_size == 0) {
			printf("Warning: BTS file is empty !");
			return 0;
		}
	}
	else {
		printf("Sending script to bluetooth socket\n");
	}
...
int texas_init(int fd, int *speed, struct termios *ti)
{
	...
	do {
		n = write(fd, cmd, 4);
		if (n < 0) {
			printf("Failed to write init command (READ_LOCAL_VERSION_INFORMATION)");
			return -1;
		}
		if (n < 4) {
			printf("Wanted to write 4 bytes, could only write %d. Stop\n", n);
			return -1;
		}
		/* Read reply. */
		if (read_hci_event(fd, resp, 100) < 0) {
			printf("Failed to read init response (READ_LOCAL_VERSION_INFORMATION)");
			return -1;
		}
	...
	/* Verify manufacturer */
	if (! is_it_texas(resp)) {
		printf("ERROR: module's manufacturer is not Texas Instruments\n");
		return -1;
	}

	printf("Found a Texas Instruments' chip!\n");
