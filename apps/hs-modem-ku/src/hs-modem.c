							...
							unsigned long arg)
{
	...
	else if( cmd == HSMODEM_CALIB_RX(hsModem->devt))
	{
		...
		//printk ("XXX (HSMODEM_CALIB_RX) rx_frame_bytes: %d\n", hsModem->modem_rx_param.rx_frame_bytes);
	...
	else if( cmd == HSMODEM_CALIB_TX(hsModem->devt))
	{
		...
		//printk ("XXX (HSMODEM_CALIB_TX) tx_frame_bytes: %d\n", hsModem->modem_tx_param.tx_frame_bytes);
...
static irqreturn_t hsModem_irq_tx_handler(int irq, void *arg)
{
	...
	static struct timeval first_time={0};
	static struct timeval second_time={0};

	disable_irq(hsModem->irq_rx);

	do_gettimeofday (&second_time);
	//printk ("Time ellapsed since last (ns) : %ld\n", timeval_to_ns(&second_time) - timeval_to_ns(&first_time));
	printk ("Time ellapsed since last (micro-s) : %ld\n", ((second_time.tv_sec * 1000000) +  second_time.tv_usec)- ((first_time.tv_sec * 1000000) +  first_time.tv_usec));
...
static int hsModem_handle_tpr_intr(struct hsModem_t* hsModem)
{
	...
	//static uint_32_t time_called = 0;
	...
	//time_called++;
	//printk ("hsModem_handle_tpr_intr :%d | %d\n", time_called, hsModem->tdd.tsToTx);
...
static int hsModem_write_pck(struct hsModem_t* hsModem)
{
	...
	//printk ("Write 1 message with size of: %d bytes\n", len);
